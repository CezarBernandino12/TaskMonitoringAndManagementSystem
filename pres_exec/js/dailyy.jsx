...
function DailyReportPage() {
    ...
    const chartBarLabels = selectedEmp
        ? [selectedEmp.name]
        : employees.map((e) => e.name);

    const chartCompleted = selectedEmp
        ? [selectedEmp.completed]
        : employees.map((e) => e.completed);

    const chartOngoing = selectedEmp
        ? [selectedEmp.ongoing]
        : employees.map((e) => e.ongoing);

    const chartOverdue = selectedEmp
        ? [selectedEmp.overdue]
        : employees.map((e) => e.overdue);
    ...
    return (
        <div className="daily-report-page p-4">
                    ...
                    <table className="table table-bordered table-hover align-middle">
                        ...
                        <tbody>
                            {employees.map((emp) => {
                                ...
                                const rate =
                                    total > 0
                                        ? Math.round((emp.completed / total) * 100)
                                        : 0;
