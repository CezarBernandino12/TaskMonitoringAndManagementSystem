...
// ====================================================================
// INPUT VALIDATION (ANNUAL)
// ====================================================================
$employeeId = $_GET['employee_id'] ?? null;
$year       = $_GET['year'] ?? null;

// Optional manual override
$start      = $_GET['year_start'] ?? null;
$end        = $_GET['year_end'] ?? null;
...
// If year is provided → auto-generate range
...
if (!$start || !$end) {
    http_response_code(400);
    echo json_encode(['error' => 'Missing year or date range']);
    exit;
}
...
$tasksSql = "
    ...
    WHERE t.assigned_to = ?
    AND (
        -- Completed within selected year
        (
