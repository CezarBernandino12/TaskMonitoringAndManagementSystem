...
function SupervisorDashboard() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const safeStaff = extractArray(staffJson, ["staff", "data", "results"]);
    ...
    const completedTasks = Array.isArray(tasks)
        ? tasks.filter((t) => normalizeStatus(t?.status) === "Completed").length
        : 0;
    const ongoingTasks = Array.isArray(tasks)
        ? tasks.filter((t) => normalizeStatus(t?.status) === "Ongoing").length
        : 0;
