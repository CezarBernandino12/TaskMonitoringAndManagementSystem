...
function ensureChartJs() {
    ...
    return new Promise((resolve, reject) => {
        const existing = document.querySelector('script[data-quarterly-chartjs-loader="true"]');
        ...
        script.async = true;
        script.dataset.quarterlyChartjsLoader = "true";
...
function QuarterlyReportPage() {
    ...
    useEffect(() => {
        ...
        ensureChartJs()
            .then(() => {
                if (cancelled || !deptChartRef.current) return;
    ...
    useEffect(() => {
        ...
        ensureChartJs()
            .then(() => {
                if (cancelled || !monthlyChartRef.current) return;
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    footer: (items) => {
                                        ...
                                        const total = comp + (trendOngoing[idx] || 0) + (trendOverdue[idx] || 0);
    ...
    useEffect(() => {
        ...
        ensureChartJs()
            .then(() => {
                if (cancelled || !donutChartRef.current) return;
                        ...
                        datasets: [
                            {
                                ...
                                backgroundColor: total > 0
                                    ? [STATUS_COLORS.Completed, STATUS_COLORS.Ongoing, STATUS_COLORS.Overdue]
                                    : [chartTheme(themeMode).isDark ? "#334155" : "#e5e7eb"],
                                borderWidth: 4,
    ...
    useEffect(() => {
        if (!employeeChartRef.current) return;

        let cancelled = false;

        ensureChartJs()
            .then(() => {
                if (cancelled || !employeeChartRef.current) return;
    ...
    return (
        <div className="dr-page">
            <div className="dr-card qr-toolbar-card">
                <div className="dr-card-head qr-toolbar-row">
                    ...
                    <div className="qr-toolbar-actions">
                        ...
                        <div className="qr-dept-dropdown" ref={deptDropdownRef}>
                            <button
                                type="button"
                                className={`qr-dept-trigger ${deptOpen ? "is-open" : ""}`}
            ...
            <div className="dr-card qr-block-gap">
                ...
                <div className="ar-chart-wrap ar-chart-wrap--dept qr-chart-lg">
                    <canvas ref={deptChartRef}></canvas>
                </div>
            </div>

            <div className="qr-split-grid qr-top-grid-annual">
                <div className="dr-card qr-card--monthly">
                    <div className="dr-card-head">
                        <div>
                    ...
                    <div className="ar-quarter-chip-row qr-month-chip-row">
                        {monthlyTrend.map((month) => {
                            const completed = safeNum(month.completed);
                            const total =
                                completed +
                                safeNum(month.ongoing) +
                                safeNum(month.overdue);

                            return (
                                <span className="ar-quarter-chip" key={month.month_name}>
                    ...
                    <div className="ar-chart-wrap ar-chart-wrap--quarter qr-chart-md">
                        <canvas ref={monthlyChartRef}></canvas>
                    </div>
                </div>

                <div className="dr-card qr-card--status-mix">
                    <div className="dr-card-head">
                        <div>
                    ...
                    <div className="dr-donut-stack">
                        <div className="dr-donut-shell">
                            <canvas ref={donutChartRef} className="dr-donut-chart ar-donut-chart"></canvas>
            ...
            <div className="dr-card qr-block-gap">
                ...
                <div
                    className="ar-chart-wrap ar-chart-wrap--employee qr-chart-hbar"
                    style={{
...
const mountNode = document.getElementById("quarterlyReportRoot") || document.getElementById("root");
