...
$start      = $_GET['quarter_start'] ?? $_GET['week_start'] ?? null;
$end        = $_GET['quarter_end']   ?? $_GET['week_end']   ?? null;
...
// ====================================================================
// FETCH TASKS (QUARTERLY LOGIC)
// ====================================================================
//
// LOGIC:
// - Completed → only tasks completed within the selected quarter
// - Ongoing / Overdue → still included (active tasks)
//
// Same behavior as weekly/monthly — consistent UX
// ====================================================================
$tasksSql = "
    SELECT
        t.id,
        ...
        -- Derived status (aligned with frontend)
        CASE
    ...
    WHERE t.assigned_to = ?
    AND (
        -- Completed within selected quarter
        (
